...
const Chat = () => {
  const { id } = useParams();
  const { chats, addChat, updateChat } = useContext(GeminiContext);
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    setLoading(true);
    const url = id ? `http://localhost:3001/api/conversation/${id}` 
                   : "http://localhost:3001/api/conversation";
    const method = id ? "PUT" : "POST";
    
    // Ensure prompt and model are correctly retrieved
    const prompt = messageRef.current.value;
    const model = id ? chat.model : modelRef.current.value;
